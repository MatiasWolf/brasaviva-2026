...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<Usuario> {
    ...
    if (!perfil) {
      ...
      throw new AuthError('No encontramos tu perfil. Contactá al restaurante.');
    }
  ...
  private async validarEstado(perfil: Usuario): Promise<void> {
    const esCliente = ROLES_CLIENTE.includes(perfil.roles?.nombre ?? '');
    const bloqueado =
      perfil.estado === 'rechazado' || (esCliente && perfil.estado === 'pendiente');
